...
def findNextHorizontalMoves(sol_path, puzzle, k, start_car, end_car, fuels, heuristic):
    new_nodes = []
    ...
    for position in range(end_car + 1, 6):
        ...
        if displacement <= fuels[car_type]:  # to check the car has enough fuel to move
            ...
            for newIndex in reversed(
                    range(start_car, position + 1)):  # moving the car, by sending each element left at a time and then
...
def findNextVerticalMoves(sol_path, puzzle, k, start_car, end_car, fuels, heuristic):
    new_nodes = []
    car_length = end_car - start_car + 1
    car_type = puzzle[start_car][k]
        ...
        for column in range(len(puzzle[row])):
            if (puzzle[row][column] not in cars_moved) and (puzzle[row][column] != "."):
                ...
                if (column +1 != 6) and (puzzle[row][column] == puzzle[row][column + 1]):  # horizontal car *
                    ...
                    for i in range(column, len(puzzle[row])):  # The loop will go till index 5 because there are total 6 elements in a row. *
                    ...
                    horizontal_children = findNextHorizontalMoves(sol_path, puzzle, row, column, finishing_index_horizontal, fuels, heuristic)
                    ...
                    for j in range(row, len(puzzle)):  # The loop will go till index 5 because there are total 6 rows. *
                    ...
                    vertical_children = findNextVerticalMoves(sol_path, puzzle, column, row, finishing_index_vertical, fuels, heuristic)
